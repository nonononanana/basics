...
对**所有11个调制类型**（包括已知类和未知类），会生成3张图片：

1. **去噪前图片** (`<调制类型名称>_noisy.png`)
   - 时域波形（I/Q通道）
   - I/Q星座图

2. **去噪后图片** (`<调制类型名称>_denoised.png`)
   - 时域波形（I/Q通道）
   - I/Q星座图

3. **残差图片** (`<调制类型名称>_residual.png`)
   - 时域波形（I/Q通道）
   - I/Q星座图
   - 残差幅度
   - 残差功率谱密度（PSD）

**总计：33张图片（11个调制类型 × 3张图片/类型）**

**注意：** 对于未知类（模型未训练过的调制类型），去噪会使用模型的第一个已知类作为参考，这样可以清楚地看到当模型遇到未知类型时会发生什么。
